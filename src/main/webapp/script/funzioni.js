...
function validaUsername() {
    ...
    if (input.value.length >= 6 && input.value.match(/^[0-9a-zA-Z]+$/)) {
        ...
        xmlHttpReq.open("GET", "verifica_username_servlet?username=" + encodeURIComponent(input.value), true);
        xmlHttpReq.send();
    } else {
        input.style.border = borderNo;
        usernameOk = false;
...
function validaEmail() {
    ...
    if (input.value.match(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w+)+$/)) {
        ...
        xmlHttpReq2.open("GET", "verifica_email_servlet?email=" + encodeURIComponent(input.value), true);
        xmlHttpReq2.send();
    }else {
        input.style.border = borderNo;
        emailOk = false;
...
function ricerca1(valore) {
    var datalist = document.getElementById("ricerca_datalist1");
    ...
    xhttp.open("GET","ricerca_ajax_servlet?cellulare=" +encodeURIComponent(valore),true);
    xhttp.send();
}

function ricerca2(valore) {
    var datalist = document.getElementById("ricerca_datalist2");
    if(valore.length == 0) {        //nessun suggerimento, l'utente ha iniziato a digitare e poi ha cancellato
        datalist.innerHTML = "";
        return;
    }
...
function openBarra(){
    document.getElementById("barra_responsive").style.width = "220px";
}

function closeBarra(){
    document.getElementById("barra_responsive").style.width = "0px";
}
